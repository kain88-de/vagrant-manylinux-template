...
sudo mv devtools-2.repo /etc/yum.repos.d/devtools-2.repo
...
sudo chown vagrant:vagrant -R /opt/python
...
sudo ln -s $PY35_BIN/auditwheel /usr/local/bin/auditwheel
...
sudo yum -y install ${MANYLINUX1_DEPS}
sudo yum -y clean all > /dev/null 2>&1
yum list installed
# we don't need libpython*.a, and they're many megabytes
sudo find /opt/_internal -name '*.a' -print0 | xargs -0 rm -f
# Strip what we can -- and ignore errors, because this just attempts to strip
# *everything*, including non-ELF files:
sudo find /opt/_internal -type f -print0 \
    | xargs -0 -n1 strip --strip-unneeded 2>/dev/null || true
# We do not need the Python test suites, or indeed the precompiled .pyc and
# .pyo files. Partially cribbed from:
#    https://github.com/docker-library/python/blob/master/3.4/slim/Dockerfile
sudo find /opt/_internal \
